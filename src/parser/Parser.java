...
public class Parser extends JFrame {
  ...
  public LogFormat getLogFormat() { return logFormats[parserType.value]; }
  ...
  public static DataTree getDataMap(Data data, Limits limits) {
  ...
  public boolean set_log_format(BufferedReader bufferedReader) throws IOException {
    ...
    if (type == NONE) {
      IOManager.asError("Logger format unsupported!");
    } else if (type == WIND_DIR_SPEED_TEMP_HUM_BAR_RAIN) {
      //TODO: do it smarter
      //skip two lines
      mergedInput.append(bufferedReader.readLine() + "\n");
      mergedInput.append(bufferedReader.readLine() + "\n");
    }
  ...
  public void parse(BufferedReader bufferedReader, LogFormat logFormat) throws IOException {
  ...
  public boolean parse(File file){
    try {
      ...
      if (!set_log_format(bufferedReader)) return false;

      LogFormat logFormat = getLogFormat();
      logFormat.configure(file.getName());
      setVisible(true);
      parse(bufferedReader, logFormat);
